...
Once Go is on the `PATH`, the action exposes the most commonly needed `go env` variables as outputs, so workflows don't have to query them in duplicated `bash`/`pwsh` steps — one expression works on Linux, macOS, and Windows. Each one keeps the name `go env` prints, so `go env GOMODCACHE` on the runner and `outputs.GOMODCACHE` refer to the same thing. The values are a snapshot from setup time: anything a later step changes, such as setting `GOOS`/`GOARCH` to cross-compile, is not reflected.
...
One further value is computed by the action rather than read from `go env`, so it keeps the kebab-case naming the action's other outputs use:

| Output | Notes |
| --- | --- |
| `go-bin-path` | `GOBIN` if set, otherwise `bin` under the first `GOPATH` entry. Cross-compiled binaries go one level deeper, in `$GOPATH/bin/$GOOS_$GOARCH` |
